...
from (select org_id,
      ...
      group by org_id,
         ...
         left join (select id,
                    ...
                    where dt = '20250717') region on city_id = region.id;
...
from (select order_id,
      ...
      from (select order_id,
           ...
           (select id org_id,
                   ...
                   region_id
            from dim_organ_full
            where dt = '2025-07-17') organ
           on detail.sender_district_id = organ.region_id
               left join
           (select id,
                   parent_id
            from dim_region_full
            where dt = '20250717') district
...
from (select org_id,
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where dt = '20250717') city_for_level1
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where dt = '20250717') province_for_level1
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where dt = '20250717') province_for_level2
...
where dt >= date_add('2025-07-17', -recent_days + 1)
group by org_id,
    org_name,
    city_id,
    city_name,
...
from (select recent_days,
     ...
     (select id,
             ...
             region_name
      from dim_organ_full
      where dt = '2025-07-17') first
on aggregated.org_id = first.id
    left join
    (select id,
    parent_id
    from dim_region_full
    where dt = '20250717') parent
    ...
    line_name
    from dim_shift_full
    where dt = '20250717') for_line_name
    on shift_id = for_line_name.id
    left join (select id,
    truck_model_type,
    truck_model_type_name
    from dim_truck_full
    where dt = '20250717') truck_info on truck_id = truck_info.id;
