...
region = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
...
district = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
    .select("id", "parent_id")

city = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
    .select("id", "name", "parent_id").withColumnRenamed("id", "city_id").withColumnRenamed("name", "city_name")

province = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
...
truck_info = spark.table("wl.dim_truck_full").filter(F.col("dt") == "20250717") \
...
city_for_level1 = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
    .select("id", "name", "parent_id")

province_for_level1 = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
    .select("id", "name", "parent_id")

province_for_level2 = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
    .select("id", "name")
...
final_df = filtered.groupBy("recent_days").agg(
    F.sum("order_count").alias("order_count"),
    F.sum("order_amount").alias("order_amount")
).withColumn("dt", F.lit("20250717"))  # 添加分区日期
...
parent = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
    .select("id", "parent_id")

city = spark.table("wl.dim_region_full").filter(F.col("dt") == "20250717") \
    .select("id", "name")

for_line_name = spark.table("wl.dim_shift_full").filter(F.col("dt") == "20250717") \
    .select("id", "line_name")
