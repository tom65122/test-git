...
cargo_df = spark.table("wl.ods_order_cargo").filter(
    (F.col("dt") == "20250717") & (F.col("is_deleted") == "0")
).select(
    F.col("id"),
    F.col("order_id"),
    F.col("cargo_type"),
...
joined_df = transport_task_df.alias("tt") \
    ...
    # 分区字段固定为'20250717'（与原SQL保持一致）
    F.lit("20250717").alias("dt")
)
...
# 7. 单独处理20250717分区数据
inbound_20250717_df = spark.table("wl.ods_order_org_bound").filter(
    F.col("dt") == "20250717"
).select(
    F.col("id"),
    F.col("order_id"),
...
# 8. 验证20250717分区数据量
print_data_count(inbound_20250717_df, "dwd_bound_inbound_inc（20250717分区）")

# 9. 写入20250717分区数据（覆盖模式）
inbound_20250717_df.write.mode("overwrite") \
    .partitionBy("dt") \
    .orc("/warehouse/wl/dwd/dwd_bound_inbound_inc")
...
# 7. 单独处理20250717分区数据（带删除标记过滤）
sort_20250717_df = spark.table("wl.ods_order_org_bound").filter(
    (F.col("dt") == "20250717") &
    (F.col("sort_time").isNotNull())
).select(
    F.col("id"),
...
# 8. 验证20250717分区数据量
print_data_count(sort_20250717_df, "dwd_bound_sort_inc（20250717分区）")

# 9. 写入20250717分区数据（覆盖模式）
sort_20250717_df.write.mode("overwrite") \
    .partitionBy("dt") \
    .orc("/warehouse/wl/dwd/dwd_bound_sort_inc")
...
# 7. 单独处理20250717分区数据（带删除标记过滤）
outbound_20250717_df = spark.table("wl.ods_order_org_bound").filter(
    (F.col("dt") == "20250717") &
    (F.col("outbound_time").isNotNull())
).select(
    F.col("id"),
...
# 8. 验证20250717分区数据量
print_data_count(outbound_20250717_df, "dwd_bound_outbound_inc（20250717分区）")

# 9. 写入20250717分区数据（覆盖模式）
outbound_20250717_df.write.mode("overwrite") \
    .partitionBy("dt") \
    .orc("/warehouse/wl/dwd/dwd_bound_outbound_inc")
